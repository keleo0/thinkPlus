...
{
	char usage[]="usage: logserver [-p cfgpath] [-c cfgfile] [-d cfgdomain]";
	int lsnport;
	char lsnip[16],strlsnport[6];
	LOL_NET *net;
	LOL_CFG *cfg;
	...
	if(lol_getcfgs(cfg,cfgarray,4)<0){
		lol_showerror();
		return -1;
	}
	lol_freecfg(cfg);
	...
	lol_strgetfield(lsn_addr,':',1,strlsnport,sizeof(strlsnport));
	lsnport=atol(strlsnport);
	...
	if((lp=lol_logopen(log_path,log_name,LOL_LOG_LOCAL|lol_loggetflags(log_flags)))==NULL){
		lol_showerror();
		return -1;
	}
	lol_logsend(lp,LOL_LOG_PRINT,"system","working ...");
	while(1){
		lol_logsend(lp,0,"system","idle ...");
		if(lol_netcenter_idle(netcenter,-1)<0){
